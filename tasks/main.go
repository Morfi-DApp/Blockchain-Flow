...
func main() {
	...
	o.Tx(
		"uploadMetadata",
		WithSigner("account"),
		WithArg("name", `"First Dream"`),
		WithArg("description", `"I dreamed that I was falling from a very high place"`),
		WithArg("image", `"Alex1.png"`),
		WithArg("extra", `{"String": "Morfi Extra Metadata If needed"}`),
	...
	// Setup Bob's account to hold a Morfi NFT
	color.Red("Should be able to setup Bob's account with Morfi collection path")
	o.Tx("setupAccount",
		WithSigner("bob"),
	)
	color.Green("-----------------------------PASSED---------------------")
	...
	// Get all the Morfi NFTs owned by this account
	color.Red("Should be able to fetch all the Morfi NFTs owned by this account")
